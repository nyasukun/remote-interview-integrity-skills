...
完全な読みのモーラ比率やforced alignmentによる音素ラベルも、実現音の独立した確認にはならない。語内の母音・他子音との対応を音声側で確認する。

## 5. 音響開放

映像を見ず、48 kHz原音または可逆抽出を用いる。波形だけでなく、閉鎖による低エネルギー、広帯域burst、高域エネルギー、12 ms energy slope、spectral flux、VOTを確認する。

自動候補は未校正の保守的ヒューリスティックで絞り、曖昧なtop-1を自動確定しない。一般的な検出精度や誤検出率はこのゲートから主張できない。上位候補を音声のみのランダム順レビューへ回す。対象音素でない、releaseが分離不能、重複発話、ノイズ抑制で消失した場合は `unmeasurable` にする。
