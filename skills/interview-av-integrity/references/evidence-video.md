...
本動画の前に、静止画mockupを作る。上段原映像枠、口元inset、下段波形、release marker、playhead、判定色、注意書きを含める。
imagegenを使う場合も、実録画、原フレーム、口元crop、氏名等は添付せず、合成プレースホルダだけでレイアウトを提案する。

ユーザーが明示承認するまで本レンダリングを開始しない。変更要望があればmockupを更新する。
