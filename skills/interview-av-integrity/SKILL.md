...
5. 音響開放を映像なしで固定する。自動保守ゲートが示すのは波形上の候補であり、対象の /p,b/ と確認済みではない。音声だけの盲検資料で実現音と時刻を確認し、母音や隣接音節との区別がつかないものは `audio_unmeasurable` にする。古いeventsに対するkeyや確定注釈を、新しい連番IDへ転用しない。
