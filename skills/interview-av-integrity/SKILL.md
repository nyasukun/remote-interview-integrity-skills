...
4. /p/ を主解析、/b/ を副解析とする。「全候補」を名乗る場合は、漢字表記で隠れる音素を落とさないよう、各ASR wordに完全な `reading` / `kana` / `pronunciation` を付与してから抽出する。読みが不完全なら `literal_kana_only` と明記し、網羅性や「全件」を主張しない。ASR時刻や完全な読みのモーラ比率から得た時刻は、粗い探索アンカーにだけ使い、測定値として採用しない。
5. 音響開放を映像なしで固定する。自動保守ゲートが示すのは波形上の候補であり、語区間内にあっても対象の /p,b/ と確認済みではない。音声だけの盲検資料で実現音と時刻を確認し、母音や隣接音節との区別がつかないものは `audio_unmeasurable` にする。未確認候補を確定release markerへ転用しない。古いeventsに対するkeyや確定注釈を、新しい連番IDへ転用しない。
...
- 閉鎖を観測できないイベントでは、可視開放時刻を捏造せず `unavailable` にする。閉鎖前後の原フレームや口元測定が欠けた窓を「閉鎖なし」と判定しない。接触を確認できたフレームと、窓全体で接触がないという主張は区別する。
